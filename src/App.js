...
class App extends React.Component {
    constructor(props) {
        ...
        this.transferValues = Object.values(transferFilter.items).map(item => item.value);
        this.allTickets = [];
    ...
    componentDidMount() {
        fetch('https://front-test.beta.aviasales.ru/search')
            .then(res => res.json())
            .then(({ searchId }) => this.setState({ searchId }, () => this.loadTickets()));
    }
    ...
    filterTickets( ) {
        ...
        const selectedFiltersNumbers = checkedFiltersValues.map(val => transferFilter.items[val].number);
    ...
    handleChangeButtonFilter = (value) => {
        ...
        this.setState({ selectedFilter: value }, () => this.sortTickets({ tickets: displayedTickets }));
    };
  ...
  handleChangeTransferFilter = value => {
    ...
    const foundFilterIdIndex = checkedFiltersValues.findIndex(filterId => value === filterId);
    ...
    if(isAllFilterSelected) {
        ...
        newValues = isIdInArr ? checkedFiltersValues.filter(filterId => ![value, allFilterValue].includes(filterId)) : [...checkedFiltersValues, value]

        if(newValues.length === (Object.entries(transferFilter.items).length - 1)) {
  ...
  render() {
        const { checkedFiltersValues, selectedFilter, displayedTickets, isDataLoading } = this.state;
            ...
            <div className="App">
                ...
                <div className={"App__content"}>
                    ...
                    <div className={'App__filter-tickets'}>
                        ...
                        />
                        {!isDataLoading && !displayedTickets.length ? <p>Билеты не найдены</p> : (
                            <TicketsBlock
